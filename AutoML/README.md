...
|Misson|내용|
|:---:|---|
|**1**|Fashion MNIST 데이터셋을 사용하여 Toy model에 hyperparamter(batch_size, epochs, Learning rate 등)를 search하는 코드 작성 후 Acc를 maximize하는 configuration을 탐색|
|**2**|모델의 파라미터 갯수는 최소화(Minimize), 성능(Acc)는 최대화(Maximize)하는 Hyperparater configuration을 찾는 코드를 작성|
...
|**4**|AutoML의 학습 시간을 줄이는 방법에는 어떤 것들이 있을지 생각|
