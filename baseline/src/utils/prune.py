...
def model_structured_prune_for_shufflenet_v2(
    model: nn.Module, conv_prun_norm: int = 1, conv_prun_rate: float = 0.2,
):
...
if __name__ == "__main__":
    ...
    BASE_PATH = path.join(ROOT_PATH, "p4-opt-5-vibrhanium-/baseline")
    MODEL_PATH = path.join(ROOT_PATH, "input/exp/shufflenet_v205/256_shufflenet.pt")
    ...
    # -- 모델 선언
    # model_name = "Shufflenet_v205"

    # model = getattr(__import__("src.models", fromlist=[""]), model_name)()
    import torch
    import torch.nn as nn
    import torchvision.models
    ...
    optimized_model = model_structured_prune_for_shufflenet_v2(
        model, conv_prun_rate=0.6
    )
