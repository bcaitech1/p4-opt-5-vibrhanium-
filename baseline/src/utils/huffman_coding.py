...
def huffman_encode_model(model, directory='encodings/'):
    ...
    print('-'*70)
    print(f"{'total':45} | {original_total:>10} {compressed_total:>10} {original_total / compressed_total:>10.2f}x {100 * compressed_total / original_total:>6.2f}%")
...
def huffman_decode_model(model, directory='encodings/'):
    for name, param in model.named_parameters():
        ...
        if 'weight' in name:
            ...
            # Insert to model
            param.data = torch.from_numpy(mat.toarray()).to(dev).reshape(origin_shape)
        else:
            dev = param.device
